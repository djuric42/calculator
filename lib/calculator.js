...
(function(window, undefined) {
  ...
  Stack.evaluateSum = function(lval, op, rval) {
    var result;

    switch(op) {
      case '*':
        result = lval.multiply( new BigNumber(rval) );
        break;
      case '/':
        result = lval.divide( new BigNumber(rval) );
        break;
      case '+':
        result = lval.add( new BigNumber(rval) );
        break;
      case '-':
        result = lval.substract( new BigNumber(rval) );
        break;
    }
  ...
  Stack.evaluateContext = function(context) {
    // we only need precedence ordering if the length of the context is greater than 3 (e.g. 2 + 3 * 3)
    if(context.length > 3) {
      // Calculate by order of precedence
      for(var i = 0; i < context.length; i++) {
        if(context[i] === '*' || context[i] === '/') {

          // Set the number before the operator to the calculated output
          context[i-1] = Stack.evaluateSum( new BigNumber(context[i-1]), context[i], new BigNumber(context[i+1]) );
          // Remove the operator and the number after the operator from the context
          context.splice(i, i+1)
    ...
    var lval=0, rval=0, op=false;
    if(context.length > 3) {
      for(var i = 0; i < context.length; i++) {
        // Test whether this is a number
        if( /^\d+(?:\.\d+)?$/.test(context[i]) ) {
          lval = new BigNumber( context[i] );
        } else { /* it's a operator */
          // Get the next value of the stack
          rval = context[i+1];
          lval = Stack.evaluateSum( new BigNumber(lval), context[i], new BigNumber(rval) );
  ...
  Stack.prototype.pop = function() {
    ...
    var val = Stack.evaluateContext(context);
  ...
  Calculator.prototype.defaultOperator = function(value, stackValue, stack, event) {
    ...
    if(stackValue !== '') {
      ...
      if(this.operators.indexOf(this.lastKey) === -1 && displayCalculation(op, this.lastOp)) {
        this.screen = Stack.evaluateContext( this.stack.peek() );
      }
      */
  ...
  Calculator.prototype.initialize = function() {
    ...
    this.defineInput(7); this.defineInput(8); this.defineInput(9); this.defineOperator("x")
    ...
    this.defineKey('input', "+/-", function(value, stackValue, stack, event) {
      if(this.last < 0) {
        this.last = Math.abs(this.last);
      } else {
        this.last *= -1
      }
